    ...
    async with async_session_factory() as session:
        ...
        result = await session.execute(
            select(func.count()).select_from(Candle).where(
                Candle.symbol == symbol, Candle.timeframe == "H1"
            )
        )
        ...
        if h1_count < min_needed:
            ...
            try:
                candles = await ingestor.fetch_candles(symbol, "H1", outputsize=5000)
        ...
        for tf, size in [("H4", 5000), ("D1", 5000)]:
            ...
            if tf_count < 100:
                ...
                ingestor = CandleIngestor(api_key=settings.twelve_data_api_key)
                try:
                    candles = await ingestor.fetch_candles(symbol, tf, outputsize=size)
                    count = await ingestor.upsert_candles(session, candles)
...
async def lifespan(app: FastAPI) -> AsyncGenerator[None, None]:
    ...
    logger.info("ApexQ application stopped")
...
async def root() -> RedirectResponse:
    """Send browser users to the dashboard."""
