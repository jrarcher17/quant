...
async def status(
    ...
    try:
        result = await session.execute(
            select(func.max(Candle.timestamp)).where(Candle.symbol == symbol)
        )
    ...
    try:
        result = await session.execute(
            select(func.max(Signal.created_at)).where(Signal.symbol == symbol)
        )
...
async def debug_backfill(timeframe: str, outputsize: int = 5000):
    ...
    try:
        ...
        async with async_session_factory() as session:
            candles = await ingestor.fetch_candles(symbol, timeframe, outputsize=outputsize)
...
async def debug_api_test():
    ...
    results = {"api_key_preview": key_preview, "symbol": symbol}
    ...
    try:
        async with httpx.AsyncClient(timeout=10.0) as client:
            resp = await client.get(
                ...
                params={"symbol": api_symbol, "apikey": key},
            )
    ...
    try:
        ...
        async with async_session_factory() as session:
            count = await ingestor.fetch_and_store(session, symbol, "H1")
