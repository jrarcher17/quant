...
from app.services.trade_settings import TradeSettingsPayload, get_trade_settings
...
class RiskManager:
    ...
    async def check(
        ...
        daily_breached, daily_pnl = await self._check_daily_loss(
            session,
            trade_settings,
        )
        ...
        for candidate in candidates:
            ...
            at_limit, active_count = await self._check_concurrent_limit(
                session,
                trade_settings,
            )
            if at_limit:
                logger.info(
                    ...
                    count=active_count,
                    max=trade_settings.max_concurrent_signals,
                ...
                results.append((
                    ...
                    RiskCheckResult(
                        ...
                        rejection_reason=(
                            ...
                            f"{trade_settings.max_concurrent_signals} active"
                        ),
                        daily_pnl=daily_pnl,
                    ),
                ))
    ...
    async def _check_daily_loss(
        ...
        is_breached = daily_loss_pct <= -trade_settings.daily_loss_limit_pct
        ...
        logger.debug(
            ...
            pct=daily_loss_pct,
            limit=trade_settings.daily_loss_limit_pct,
    ...
    async def _check_concurrent_limit(
        ...
        return (active_count >= trade_settings.max_concurrent_signals, active_count)
