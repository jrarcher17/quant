...
class StrategySelector:
    ...
    async def check_h4_confluence(
        ...
        stmt = (
            select(Candle)
            .where(Candle.symbol == get_settings().trading_symbol, Candle.timeframe == "H4")
    ...
    async def _fetch_latest_results(
        ...
        for strat in strategies:
            ...
            for window in [14, 30, 60, 7, None]:
                bt = await self._latest_result_for(
                    session, strat.id, symbol=symbol, window_days=window
                )
                if bt is not None:
                    break
            if bt is not None:
    ...
    async def _detect_volatility_regime(
        ...
        stmt = (
            select(Candle)
            .where(Candle.symbol == get_settings().trading_symbol, Candle.timeframe == "H1")
            .order_by(Candle.timestamp.desc())
            .limit(720)
        )
        result = await session.execute(stmt)
