...
- Python 3.12
- Node.js 18+ and npm (for Better Auth)
...
```bash
# 1. Clone
git clone https://github.com/vaughanf1/ApexQ.git
cd ApexQ
...
cd auth && npm run migrate && npm run seed && cd ..
...
Visit:
- Login/dashboard → http://localhost:8080/
...
| `TRADING_SYMBOL` | no | `XAUUSD` | Trading symbol to ingest, backtest, and display |
| `BETTER_AUTH_SECRET` | yes | — | Secret used by Better Auth for sessions. Generate with `cd auth && npx auth secret`. |
| `BETTER_AUTH_URL` | yes | `http://127.0.0.1:8080` | Public URL for the FastAPI app that proxies `/api/auth/*`. |
| `AUTH_PORT` | no | `3000` | Port for the local Better Auth Node service. |
| `AUTH_SERVICE_URL` | no | `http://127.0.0.1:3000` | Internal URL for the local Better Auth Node service. |
...
BETTER_AUTH_SECRET=replace_with_npx_auth_secret
BETTER_AUTH_URL=http://127.0.0.1:8080
AUTH_PORT=3000
AUTH_SERVICE_URL=http://127.0.0.1:3000
BETTER_AUTH_SEED_USERS=admin@example.com:change-this-password:ApexQ Admin
TELEGRAM_BOT_TOKEN=
TELEGRAM_CHAT_ID=
DIDWW_SMS_USERNAME=
DIDWW_SMS_PASSWORD=
...
```bash
docker build -t apexq .
docker run --rm -p 8080:8080 --env-file .env apexq
```
